...
struct parameters {
	...
	double (*fitnessFunction)(struct parameters *params, struct chromosome *chromo, struct dataSet *dat, double threshold);
...
/* fitness function */
static double supervisedLearning(struct parameters *params, struct chromosome *chromo, struct dataSet *data, double threshold);
...
*/
DLL_EXPORT void setCustomFitnessFunction(struct parameters *params, double (*fitnessFunction)(struct parameters *params, struct chromosome *chromo, struct dataSet *data, double threshold), char const *fitnessFunctionName) {
...
DLL_EXPORT double getChromosomeOutput(struct chromosome *chromo, int output) {
	...
	if (output < 0 || output > chromo->numOutputs-1) {
...
*/
DLL_EXPORT void setChromosomeFitness(struct parameters *params, struct chromosome *chromo, struct dataSet *data, double threshold) {
	...
	fitness = params->fitnessFunction(params, chromo, data, threshold);
...
*/
DLL_EXPORT void setChromosomeFitnessValidation(struct parameters *params, struct chromosome *chromo, struct dataSet *data, double threshold) {
...
DLL_EXPORT struct chromosome* runCGP(struct parameters *params, struct dataSet *dataTrain, struct dataSet *dataValid, int numGens, unsigned int * seed) 
{
	...
	setChromosomeFitnessValidation(params, bestChromo, dataValid, 0.5);
	...
	for (i = 0; i < params->mu; i++) 
	{
		setChromosomeFitness(params, parentChromos[i], dataTrain, 0.5);
		setChromosomeFitnessValidation(params, parentChromos[i], dataValid, 0.5);
	}
	...
	for (gen = 0; gen < numGens; gen++) 
	{
		...
		for (i = 0; i < params->lambda; i++) 
		{
			setChromosomeFitness(params, childrenChromos[i], dataTrain, 0.5);
			setChromosomeFitnessValidation(params, childrenChromos[i], dataValid, 0.5);
		}
...
DLL_EXPORT struct chromosome* runCGPDE_IN(struct parameters *params, struct dataSet *dataTrain, struct dataSet *dataValid, int numGens, unsigned int * seed) 
{
	...
	{		
		...
		*/
		setChromosomeFitnessValidation(params, childrenChromos[best_i], dataValid, 0.5);
...
DLL_EXPORT struct chromosome ** runDE(struct parameters *params, struct chromosome *chromo, struct dataSet *dataTrain, struct dataSet *dataValid, int type, unsigned int * seed)
{
	...
	for(t = 0; t < maxIter; t++)
	{
		...
		for(i = 0; i < NP; i++)
		{
    		...
    		setChromosomeFitness(params, DEChromo_u->chromo, dataTrain, 0.5);
...
DLL_EXPORT struct chromosome * getBestDEChromosome(struct parameters *params, struct chromosome ** chromos, struct dataSet * dataValid, int typeCGPDE, unsigned int * seed)
{
	...
	if(typeCGPDE == 3) // OUT-V
	{
		...
		{
			setChromosomeFitnessValidation(params, chromos[i], dataValid, 0.5);
		}
	}
...
DLL_EXPORT struct DEChromosome ** initialiseDEPopulation(struct parameters *params, struct chromosome *chromo, struct dataSet *data, int type, unsigned int * seed)
{
	...
	setChromosomeFitness(params, DEChromos[0]->chromo, data, 0.5);
    ...
    for(i = 1; i < NP; i++)
    {
    	...
    	setChromosomeFitness(params, DEChromos[i]->chromo, data, 0.5);
	}
...
DLL_EXPORT struct DEChromosome * initialiseDEChromosome(struct parameters *params, struct chromosome *chromo, struct dataSet *data, unsigned int * seed)
{
    ...
    setChromosomeFitness(params, DEChromo->chromo, data, 0.5);
...
/*
	Get the threshold to assess the: accuracy, G-mean, and F-score.
*/
DLL_EXPORT double getThreshold(struct parameters *params, struct chromosome * chromo, struct dataSet * data)
{
	// if the objective function is not AUC, then threshold = 0.5
	if(strcmp(params->fitnessFunctionName, "AUC") != 0) 
	{
		return 0.5;
	}
    ...
    if(getNumChromosomeOutputs(chromo) != 1){
        printf("Error: the number of chromosome outputs must be 1 for binary classification.\n");
        printf("Terminating.\n");
        exit(0);
    }
    ...
    // sort outputs and labels with respect to the outputs
	double aux;

	for (i = 0; i < numDataSetSamples; i++) 
	{
		for (j = i; j < numDataSetSamples; j++) 
		{
	...
	int threshold_index;
	int best_threshold_index = 0;
	double lower_distance = DBL_MAX;
	double * TPR = (double *)malloc(numDataSetSamples * sizeof(double));
	double * FPR = (double *)malloc(numDataSetSamples * sizeof(double));

	for(threshold_index = 0; threshold_index < numDataSetSamples; threshold_index++)
	{
		unsigned long int majority_counter = 0, minority_counter = 0;

		for(i = 0; i < threshold_index; i++)
		{
			if(labels[i] == 1.0)
			{
				minority_counter++;
		...
		TPR[threshold_index] = (double)minority_counter / minority_length;
		FPR[threshold_index] = (double)majority_counter / majority_length;

		// find the best threshold
		double current_distance = sqrt( ( (0 - FPR[threshold_index])*(0 - FPR[threshold_index]) ) + ( (1 - TPR[threshold_index])*(1 - TPR[threshold_index]) ) );
	...
	double best_threshold = outputs[best_threshold_index];
...
*/
DLL_EXPORT double calculateAuc(double * output, double * label, int size, unsigned long int majority_length, unsigned long int minority_length)
{	
	int i, j;
    if(majority_length == 0 || minority_length == 0)
    {
    	printf("Error AUC!\n");
    	exit(1);
    }
    ...
    int index_minority = 0, index_majority = 0;
	double * minority = (double *) malloc(minority_length * sizeof(double));
	double * majority = (double *) malloc(majority_length * sizeof(double));

	for(i = 0; i < size; i++)
	{
		if(label[i] == 1.0) // POSITIVE
		{
			minority[index_minority] = output[i];
			index_minority++; 
		}
		else if(label[i] == 0.0) // NEGATIVE
		{
			majority[index_majority] = output[i];
			index_majority++;
		}
	}
	...
	long double auc = 0.0;
	for(i = 0; i < minority_length; i++)
	{
		for(j = 0; j < majority_length; j++)
		{
			double diff = minority[i] - majority[j];
	...
	auc = (long double)auc / (minority_length * majority_length);
